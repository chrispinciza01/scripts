...
def simulate_typing():
    """Randomly simulates pressing space or typing random keys without interfering with work."""
    if random.random() < 0.4:  # 40% chance of a key event
        if random.random() < 0.7:
            pyautogui.press("space")  # Simulate space press
            print("Simulated space key press.")
        else:
            random_text = ''.join(random.choices("abcdefghijklmnopqrstuvwxyz", k=random.randint(2, 5)))
            pyautogui.write(random_text, interval=random.uniform(0.1, 0.3))  # Simulate slow typing
            print(f"Simulated typing: {random_text}")
...
def move_mouse_naturally():
    ...
    while True:
        ...
        if random.random() < 0.3:  # 30% chance of simulating typing every cycle
            simulate_typing()
        ...
        # Occasionally right-click
        if random.random() < 0.2:  # 20% chance of right-clicking
            pyautogui.rightClick()
            print("Simulated right-click.")

        # Wait a random time before the next action
        sleep_time = random.randint(12, 45)  # Increased randomness
